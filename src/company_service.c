...
static void print_company(FILE *out, const InternationalCompany *c, size_t index) {
    fprintf(out, "[%zu] %s | site: %s | employees: %d | branches(EU): %d | market years: %d\n",
            index, c->name, c->website, c->employees, c->branches_europe, c->market_years);
    fprintf(out, "    HQ: %s, %s, %s, %s, %s, %s, house %s, apt %s\n",
...
void company_write_with_employees_gt(FILE *out, const InternationalCompany *arr, size_t n, int threshold) {
    ...
    if (!any) fprintf(out, "No companies match the condition.\n");
}
